<?php
$pageTitle='Data Kematian';
require_once __DIR__.'/../../includes/header.php';
require_role(['super_admin','admin_rtrw','ketua_rtrw']);
$rows=$pdo->query("SELECT * FROM kematian ORDER BY tanggal_meninggal DESC")->fetchAll();
$canEdit=can('manage_master');
?>
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    <span><i class="fa-solid fa-cross me-1"></i> Daftar Kematian</span>
  ...
  <div class="card-body">
    <table class="table table-hover dt">
      <thead><tr><th>No</th><th>NIK</th><th>Nama</th><th>Tgl Meninggal</th><th>Tempat</th><th>Penyebab</th><th>Aksi</th></tr></thead>
      <tbody><?php foreach($rows as $i=>$r): ?>
      <tr>
        <td><?= $i+1 ?></td><td><?= e($r['nik']) ?></td><td><?= e($r['nama']) ?></td>
        <td><?= tanggal_id($r['tanggal_meninggal']) ?></td>
        <td><?= e($r['tempat_meninggal']) ?></td><td><?= e($r['penyebab']) ?></td>
        <td><?php if($canEdit): ?>
...
<div class="modal-header"><h5 class="modal-title" id="mTitle">Tambah Kematian</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
<div class="modal-body"><div class="row g-3">
  ...
  <div class="col-md-6"><label class="form-label">Tanggal Meninggal</label><input type="date" name="tanggal_meninggal" id="f_tgl" class="form-control"></div>
  <div class="col-md-6"><label class="form-label">Tempat Meninggal</label><input name="tempat_meninggal" id="f_tmp" class="form-control"></div>
  <div class="col-12"><label class="form-label">Penyebab</label><input name="penyebab" id="f_pny" class="form-control"></div>
...
<script>
function resetForm(){['f_id','f_nik','f_nama','f_tgl','f_tmp','f_pny','f_ket'].forEach(id=>document.getElementById(id).value='');document.getElementById('f_act').value='add';document.getElementById('mTitle').innerText='Tambah Kematian';}
function setV(id,v){document.getElementById(id).value=v??'';}
function editRow(r){document.getElementById('f_act').value='edit';document.getElementById('mTitle').innerText='Edit Kematian';
  setV('f_id',r.id);setV('f_nik',r.nik);setV('f_nama',r.nama);setV('f_tgl',r.tanggal_meninggal);setV('f_tmp',r.tempat_meninggal);setV('f_pny',r.penyebab);setV('f_ket',r.keterangan);
