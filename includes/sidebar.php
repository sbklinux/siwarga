...
<aside class="app-sidebar">
  ...
  <nav class="nav flex-column px-2 pb-4">
    ...
    <?php if ($r === 'super_admin' || $r === 'ketua_rtrw'): ?>
      <?php
        $pendingReset = (int)$pdo->query("SELECT COUNT(*) FROM password_reset_requests WHERE status='pending'")->fetchColumn();
      ?>
      <a class="nav-link<?= dir_active('reset',$currentDir) ?>" href="<?= url('modules/reset/index.php') ?>">
        <i class="fa-solid fa-key"></i> Reset Password
        <?php if ($pendingReset > 0): ?><span class="badge bg-danger ms-auto"><?= $pendingReset ?></span><?php endif; ?>
      </a>
    <?php endif; ?>
    ...
    <?php if ($r === 'super_admin'): ?>
      ...
      <a class="nav-link<?= dir_active('pengaturan',$currentDir) ?>" href="<?= url('modules/pengaturan/index.php') ?>"><i class="fa-solid fa-gear"></i> Pengaturan Kop</a>
